...
<script type="text/javascript" src="./js/jquery-3.4.1.min.js"></script>
	...
	<div>
		<table border="1" class="table table-striped table-hover">
			...
			<c:choose>
				<c:when test="${empty MemberDto }">
					<tr>
						<td colspan="2">----------로그인을 해야 댓글을
							작성하실수있습니다-------------------</td>
					</tr>
				</c:when>
				<c:otherwise>
					<tr>
						<td id="member_nickname">${MemberDto.member_nickname }</td>
						<td colspan="2"><textarea name="comment_content" rows="10"
								cols="20"></textarea>
							<button class="btn btn-default" name="insert" onclick="comment(this);">댓글작성</button>
							</td>
					</tr>
				</c:otherwise>
			</c:choose>
		...
		<table border="1" class="table table-striped table-hover" id="repleList">
			<tr>
				<th>작성자</th>
				<th>내용</th>
				<th>작성일</th>
			...
			<c:choose>
				<c:when test="${empty list }">
					<tr>
						<td id='check' colspan="3">------------------등록된 댓글이 없습니다 -------------</td>
					</tr>
				</c:when>
				<c:otherwise>
						...
						<tr title="n">
							...
							<td title="n"><span class='tab'>${row.comment_titletab }</span>
								...
								<c:choose>
									...
									<c:otherwise>			
										<button class="btn btn-default" id="repleform" name="insert" onclick="reply(this);">답글달기</button>
									</c:otherwise>
								</c:choose>
								<c:if test="${!empty MemberDto }">
									<c:if test="${ MemberDto.member_nickname eq row.comment_nickname }">
											<button class="btn btn-default" name="delete" onclick="comment_del(this);">댓글삭제</button>
											<button class="btn btn-default" name="update" onclick="updateform(this);">수정하기</button>
									</c:if>
								</c:if>
							</td>
...
<script type="text/javascript">
	function reply(door) {
		...
		if (tr.attr('title') == 'n') {
			...
			"<td colspan='2'><textarea name='comment_content' rows='3' cols='10'></textarea><button name='reply' onclick='comment(this);'>글작성</button></td></tr>"))
			tr.attr('title', 'y');
		} else {
			$(door).html("답글달기")
			tr.next().remove();
	...
	function updateform(self){
		...
		if(td.attr('title') == 'n'){
			content.after($("<textarea id='content' row='3' cols='10'>"+str+"</textarea>"))
			tempStr = str;
			content.remove();
			buttons.toggle();
			button.html('수정취소');
			button.toggle();
			button.after($("<button name='updateres' onclick='updateres(this);'>수정완료</button>"));
			td.attr('title','y');
		}else {
			...
			success : function(obj){
				...
				tr.after($(
						...
						+'<td><input type="hidden" class="comment_no" value="'+obj.no+'">'+obj.nickname+'</td>'
							...
							+'<button name="insert" onclick="reply(this);">답글달기</button>'
							+'<button name="delete" onclick="comment_del(this);">댓글삭제</button>'
							+'<button name="update" onclick="updateform(this);">수정하기</button>'
						+'</td>'
						+'<td>'+obj.regdate+'</td>'
					+'</tr>'
				))
	...
	function comment(self){
		...
		$.ajax({
			...
			success : function(obj){
				...
				$("#repleList tr:first-child").after($(
						'<tr title="n">'
						+'<td><input type="hidden" class="comment_no" value="'+obj.no+'">'+obj.nickname+'</td>'
							+'<td title="n"><span class="tab">'+tab+'</span>'
							+'<span id="content">'+obj.content+'</span>'
