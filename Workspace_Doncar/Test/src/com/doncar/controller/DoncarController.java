		...
		if (command != null) {
			...
			if (command.equals("login")) {
				...
				if (command.equals("member_selectlist")) {
					...
					request.setAttribute("member_selectlist", biz.selectList());
					...
					int member_no = Integer.parseInt(request.getParameter("member_no"));
					request.setAttribute("MemberDto", biz.selectOne(member_no));
					dispatch(request, response, "memberupdate.jsp");
					...
					if (res > 0) {
						alert("회원정보 변경에 성공하셨습니다.", "main.jsp", response);
					} else {
					...
					if (res > 0) {
						request.getSession().invalidate();
						alert("회원탈퇴에 성공하셨습니다. 감사합니다.", "index.jsp", response);
					} else {
						alert("회원탈퇴에 실패하셨습니다. 어 딜 도 망 가", "main.jsp", response);
					}
				...
				if (command.equals("rentboard_selectlist")) {
					...
					if (res > 0) {
						alert("차량등록성공", "doncar.do?command=rentboard_selectlist", response);
					} else {
					...
					dispatch(request, response, "rentacc_insert.jsp");
				...
				if(res>0) {
					alert("신청성공!", "doncar.do?command=rentboard_selectone&rent_board_no="+rent_board_no, response);
					...
					if(res>0) {
						alert("승인완료", "usermain.jsp", response);
					}else {
				...
				if (command.equals("carboard_selectlist")) {
					...
					String car_board_location = request.getParameter("car_board_location");
					...
					dto.setCar_board_location(car_board_location);
					...
					if (res > 0) {
						alert("글이 삭제되었습니다.", "doncar.do?command=carboard_selectlist", response);
					} else {
					...
					if (res > 0) {
						alert("수정 완료", "doncar.do?command=carboard_selectone&car_board_no=" + car_board_no, response);
					} else {
					...
					if (res > 0) {
						alert("등록완료", "doncar.do?command=carboard_selectone&car_board_no="+car_board_no, response);
					} else {
					...
					if (res > 0) {
						alert("삭제되었습니다.", "doncar.do?command=caracc_selectlist", response);
					} else {
					...
					if (res > 0) {
						alert("수정 완료", "doncar.do?command=caracc_selectone&caracc_no=" + caracc_no, response);
					} else {
				...
				if (command.equals("freeboard_selectlist")) {
					...
					int res = biz.insert(dto);
					if (res > 0) {
						alert("글 작성 성공", "doncar.do?command=freeboard_selectlist&nowPage=1", response);
					} else {
					...
					int res = biz.update(dto);
					if (res > 0) {
						alert("글 수정 성공", "doncar.do?command=freeboard_selectlist", response);
					} else {
					...
					if (res > 0) {
						alert("글 삭제 성공", "doncar.do?command=freeboard_selectlist", response);
					} else {
				...
				if (command.equals("noticeboard_selectlist")) {
					...
					int res = biz.insert(dto);
					if (res > 0) {
						alert("글 작성 성공", "doncar.do?command=noticeboard_selectlist", response);
					} else {
					...
					if (res > 0) {
						alert("글 삭제 성공", "doncar.do?command=noticeboard_selectlist", response);
					} else {
					...
					int res = biz.update(dto);
					if (res > 0) {
						alert("글 수정 성공", "doncar.do?command=noticeboard_selectlist", response);
					} else {
